...
module.exports = {
  home: async (req, res) => {
    ...
    try {
      ...
      productCollection
        ...
        .lean()
        .then((products) => {
          console.log("Products (newest first):", products.slice(0, 5));
  ...
  userSignup: async (req, res) => {
    ...
    try {
      ...
      res.redirect(
        `/otp-verification?referralCode=${referralCode}&email=${email}`
      );
    } catch (error) {
  ...
  otpVerification3: async (req, res) => {
    ...
    if (
      ...
      return res.render("user/otp-verification", {
        messageErr: "Invalid or expired OTP. Try again.",
      });
    }
    ...
    if (otp !== req.session.tempOTP.otp) {
      return res.render("user/otp-verification", {
        messageErr: "Incorrect OTP. Register again Or Resend Again.",
      });
    }
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          res.render("user/error", {
            messageErr: "Error sending email",
            error,
          });
  ...
  viewProduct: async (req, res) => {
    ...
    try {
      ...
      const sameCatProducts = await productCollection
  ...
  quickViewProduct: async (req, res) => {
    console.log("line1099success");

    const productId = req.query.productId;
    console.log("productId", productId);
    // Find the product with the specified ID
    const product = await productCollection.find({ _id: productId }).lean();
    ...
    if (!product) {
      ...
      //   // If the product is found, return its details as a JSON response
      res.json(product);
    }
  },
  ...
  sentEmail: async (req, res) => {
    console.log("email");
    // Get the user's email and message from the request
    ...
    const mailOptions = {
      from: "userEmail",
      to: "eshoptoday.001.in@gmail.com", // Change this to your company's email address
  ...
  sortProducts: async (req, res) => {
    try {
      ...
      const proData = await productCollection.find({ deleted: false });
  ...
  categorywiseSort: async (req, res) => {
    const user = req.session.user;
    let mainImages = [];
    if (!user) {
      return res.redirect("/login");
    ...
    try {
      ...
      console.log("productsWithAverageRating", productsWithAverageRating);
      categoryCollection
        .find({ deleted: false })
        .lean()
        .then((categories) => {
